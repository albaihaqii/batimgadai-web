...
<head>
    <meta charset="UTF-8">
    <title>Data Petugas BATIM GADAI</title>
    <style>
...
<body>
    <div class="header">
        ...
        <p>Data Petugas - Dicetak pada {{ now()->format('d M Y, H:i') }} WIB</p>
    </div>

    <div class="info">
        Total Data: <strong>{{ $officers->count() }} petugas</strong>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width:12%">Tgl Bergabung</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $officer->nama }}</td>
                <td>{{ $officer->email }}</td>
                <td>{{ $officer->branch->nama ?? '-' }}</td>
                <td>{{ $officer->created_at->format('d M Y') }}</td>
                <td>
                    <span class="{{ $officer->status === 'aktif' ? 'badge-aktif' : 'badge-nonaktif' }}">
