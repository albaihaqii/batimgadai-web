...
<head>
    <meta charset="UTF-8">
    <title>Data Pimpinan BATIM GADAI</title>
    <style>
...
<body>
    <div class="header">
        ...
        <p>Data Pimpinan - Dicetak pada {{ now()->format('d M Y, H:i') }} WIB</p>
    </div>

    <div class="info">
        Total Data: <strong>{{ $admins->count() }} pimpinan</strong>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $admin->nama }}</td>
                <td>{{ $admin->email }}</td>
                <td>{{ $admin->branch->nama ?? '-' }}</td>
                <td>{{ $admin->created_at->format('d M Y') }}</td>
                <td>
                    <span class="{{ $admin->status === 'aktif' ? 'badge-aktif' : 'badge-nonaktif' }}">
