    ...
    <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-100"
        ...
        <ul class="flex flex-col h-auto overflow-y-auto custom-scrollbar">
            @if ($notifications->isEmpty())
                <li class="px-4.5 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
                    Tidak ada notifikasi baru.
                </li>
            @else
                @foreach ($notifications as $notification)
                    @php
                        $approvalUrl = route('notifications.open', $notification->id);
                    @endphp
                    <li>
                        <a class="flex gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5"
                            href="{{ $approvalUrl }}" @click="closeDropdown()">
                            <span class="relative block w-full h-10 rounded-full z-1 max-w-10">
                                <span
                                    class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-xs font-semibold text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                            ...
                            <span class="block">
                                <span class="mb-1.5 block text-theme-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    {{ strlen($notification->message) > 70 ? substr($notification->message, 0, 70) . '...' : $notification->message }}
                                </span>
