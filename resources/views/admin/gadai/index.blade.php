...
<div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div class="flex flex-col gap-4 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Transaksi Gadai</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Kelola transaksi gadai BATIM GADAI</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route(auth()->user()->role . '.transaksi.gadai') }}?export=1&search={{ request('search') }}&cabang_id={{ request('cabang_id') }}&status={{ request('status') }}"
                ...
                <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M10 2.5V13.5M10 13.5L6 9.5M10 13.5L14 9.5M3 15.5H17" stroke="" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                Unduh
            </a>
            @if(auth()->user()->role !== 'admin')
            <a href="{{ route(auth()->user()->role . '.transaksi.gadai.create') }}"
                ...
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M10 4V16M4 10H16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
                Gadai Baru
            </a>
            @endif
        </div>
    ...
    {{-- Filter --}}
    <div class="px-6 mb-3">
        <form method="GET" action="{{ route(auth()->user()->role . '.transaksi.gadai') }}" class="flex flex-wrap items-center gap-3">
            ...
            <div class="relative">
                <select name="cabang_id" onchange="this.form.submit()"
                    ...
                    @foreach($branches as $branch)
                        <option value="{{ $branch->id }}" {{ request('cabang_id') == $branch->id ? 'selected' : '' }}>{{ $branch->nama }}</option>
                    @endforeach
                </select>
            </div>
            @endif
            <select name="status" onchange="this.form.submit()"
                ...
                <option value="lelang"            {{ request('status') == 'lelang' ? 'selected' : '' }}>Lelang</option>
            </select>
            ...
            @if(request()->hasAny(['cabang_id', 'status', 'search']))
        ...
        <table class="w-full">
            <thead class="border-t border-y border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
                <tr>
                    ...
                    <th class="px-5 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Taksiran Awal</th>
                    <th class="px-5 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Nilai Pinjaman</th>
                    <th class="px-5 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Tgl Input</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @forelse($gadais as $index => $gadai)
                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                    <td class="px-5 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">{{ $gadais->firstItem() + $index }}</td>
                    <td class="px-5 py-3.5">
                        <span class="font-medium text-theme-sm text-gray-800 dark:text-white/90">{{ $gadai->no_sbg ?? '-' }}</span>
                    </td>
                    <td class="px-5 py-3.5">
                        <p class="text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ $gadai->nasabah->nama ?? '-' }}</p>
                        <p class="text-xs text-gray-400">{{ $gadai->nasabah->no_cif ?? '-' }}</p>
                    </td>
                    <td class="px-5 py-3.5">
                        <p class="text-theme-sm text-gray-800 dark:text-white/90">{{ $gadai->barang->nama_barang ?? '-' }}</p>
                        <p class="text-xs text-gray-400">{{ $gadai->barang ? ucfirst(str_replace('_', ' ', $gadai->barang->kategori)) : '-' }}</p>
                    </td>
                    ...
                    {{-- Kolom Nilai Pinjaman --}}
                    <td class="px-5 py-3.5">
                        @if($gadai->nilai_pinjaman)
                        <div>
                            <p class="text-theme-sm font-semibold text-brand-500 whitespace-nowrap">
                                Rp {{ number_format($gadai->nilai_pinjaman, 0, ',', '.') }}
                            </p>
                            @if(!empty($gadai->nilai_pinjaman_tambahan) && (int)$gadai->nilai_pinjaman_tambahan > 0)
                            <span class="mt-1 inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-theme-xs font-medium bg-warning-50 text-warning-600 dark:bg-warning-500/15 dark:text-orange-400">
                    ...
                    <td class="px-5 py-3.5">
                        ...
                        @endphp
                        <span class="rounded-full px-2 py-0.5 text-theme-xs font-medium {{ $statusConfig[$gadai->status] ?? 'bg-gray-100 text-gray-600' }}">
                    ...
                    <td class="px-5 py-3.5">
                        <div class="flex items-center gap-3">
                            ...
                            <button onclick="openDeleteModal({{ $gadai->id }}, '{{ $gadai->nasabah->nama ?? '' }}')"
                ...
                @empty
                <tr>
                    <td colspan="9" class="px-6 py-10 text-center text-sm text-gray-400 dark:text-gray-500">Belum ada data transaksi gadai</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-sm text-gray-500 dark:text-gray-400">
            Menampilkan {{ $gadais->firstItem() ?? 0 }}–{{ $gadais->lastItem() ?? 0 }} dari {{ $gadais->total() }} data
        </p>
        <div class="flex items-center gap-2">
            <a href="{{ $gadais->previousPageUrl() }}"
                class="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 {{ $gadais->onFirstPage() ? 'opacity-40 pointer-events-none' : '' }}">
            ...
            @foreach($gadais->getUrlRange(1, $gadais->lastPage()) as $page => $url)
                <a href="{{ $url }}" class="flex h-9 w-9 items-center justify-center rounded-lg text-theme-sm font-medium transition-colors {{ $page == $gadais->currentPage() ? 'bg-brand-500 text-white' : 'text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400' }}">{{ $page }}</a>
            @endforeach
            <a href="{{ $gadais->nextPageUrl() }}"
                class="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 {{ !$gadais->hasMorePages() ? 'opacity-40 pointer-events-none' : '' }}">
...
<div id="modalHapus" class="hidden fixed inset-0 z-99999 flex items-center justify-center bg-black/50">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md mx-4">
        <div class="p-6 text-center">
            ...
            <h4 class="text-lg font-semibold text-gray-800 dark:text-white mb-2">Hapus Pengajuan Gadai</h4>
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
                Apakah Anda yakin ingin menghapus pengajuan gadai atas nama <span id="deleteNama" class="font-semibold text-gray-800 dark:text-white"></span>?
            </p>
...
document.getElementById('searchInput').addEventListener('input', function() {
    ...
    document.getElementById('hiddenSearch').value = value;
...
function openDeleteModal(id, nama) {
    ...
    document.getElementById('formHapus').action = `/${role}/transaksi/gadai/${id}`;
