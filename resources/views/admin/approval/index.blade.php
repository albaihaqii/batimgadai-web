...
<div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div class="flex flex-col gap-4 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Kelola persetujuan pengajuan gadai nasabah</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            {{-- Unduh --}}
            <a href="{{ route(auth()->user()->role . '.approval.gadai') }}?export=1&search={{ request('search') }}&cabang_id={{ request('cabang_id') }}&status={{ request('status', 'menunggu_approval') }}"
    ...
    {{-- Filter --}}
    <div class="px-6 mb-3">
        <form method="GET" action="{{ route(auth()->user()->role . '.approval.gadai') }}" class="flex flex-wrap items-center gap-3">
            <input type="hidden" name="search" id="hiddenSearch" value="{{ request('search') }}">

            {{-- Filter Cabang (superadmin only) --}}
            ...
            <div class="relative">
                <select name="status" onchange="this.form.submit()"
                    class="h-9 rounded-lg border border-gray-300 bg-white px-3 pr-8 text-sm text-gray-700 shadow-theme-xs dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
                    <option value="menunggu_approval" {{ request('status', 'menunggu_approval') == 'menunggu_approval' ? 'selected' : '' }}>Menunggu Approval</option>
                    <option value="aktif" {{ request('status') == 'aktif' ? 'selected' : '' }}>Disetujui</option>
            ...
            @if(request()->filled('cabang_id') || request()->filled('search') || request()->filled('per_page') || request('status') !== null)
        ...
        <table class="w-full">
            <thead class="border-t border-y border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Taksiran Awal</th>
                    <th class="px-6 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Petugas</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @forelse($pengajuan as $index => $gadai)
                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-6 py-3.5">
                        <a href="{{ route(auth()->user()->role . '.approval.gadai.show', $gadai->id) }}"
    ...
    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ $pengajuan->previousPageUrl() }}"
                class="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 {{ $pengajuan->onFirstPage() ? 'opacity-40 pointer-events-none' : '' }}">
            ...
            <a href="{{ $pengajuan->nextPageUrl() }}"
                class="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 {{ !$pengajuan->hasMorePages() ? 'opacity-40 pointer-events-none' : '' }}">
