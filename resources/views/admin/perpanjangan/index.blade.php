...
<div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div class="flex flex-col gap-4 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Transaksi Perpanjangan</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Riwayat perpanjangan masa gadai nasabah</p>
        </div>
    ...
    {{-- Filter --}}
    <div class="px-6 mb-3">
        <form method="GET" action="{{ route(auth()->user()->role . '.transaksi.perpanjangan') }}" class="flex flex-wrap items-center gap-3">
        ...
        <table class="w-full">
            <thead class="border-t border-y border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Total Bayar</th>
                    <th class="px-6 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">JT Lama</th>
                    <th class="px-6 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">JT Baru</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">
                        {{ $item->tgl_jt_lama ? $item->tgl_jt_lama->format('d M Y') : '-' }}
                    </td>
                    <td class="px-6 py-3.5 text-theme-sm font-medium text-gray-800 dark:text-white/90">
                        {{ $item->tgl_jt_baru ? $item->tgl_jt_baru->format('d M Y') : '-' }}
                    </td>
                    <td class="px-6 py-3.5">
                    ...
                    <td class="px-6 py-3.5">
                        <a href="{{ route(auth()->user()->role . '.transaksi.perpanjangan.show', $item->id) }}"
    ...
    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-sm text-gray-500 dark:text-gray-400">
            Menampilkan {{ $perpanjangan->firstItem() ?? 0 }}–{{ $perpanjangan->lastItem() ?? 0 }} dari {{ $perpanjangan->total() }} data
        </p>
        <div class="flex items-center gap-2">
            <a href="{{ $perpanjangan->previousPageUrl() }}"
                class="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 {{ $perpanjangan->onFirstPage() ? 'opacity-40 pointer-events-none' : '' }}">
            ...
            @foreach($perpanjangan->getUrlRange(1, $perpanjangan->lastPage()) as $page => $url)
                <a href="{{ $url }}"
                    class="flex h-9 w-9 items-center justify-center rounded-lg text-theme-sm font-medium transition-colors
                    {{ $page == $perpanjangan->currentPage() ? 'bg-brand-500 text-white' : 'text-gray-700 hover:bg-brand-500 hover:text-white dark:text-gray-400' }}">
                    {{ $page }}
                </a>
            @endforeach
            <a href="{{ $perpanjangan->nextPageUrl() }}"
                class="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 {{ !$perpanjangan->hasMorePages() ? 'opacity-40 pointer-events-none' : '' }}">
