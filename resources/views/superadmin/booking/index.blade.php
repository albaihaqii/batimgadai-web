...
<div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div class="flex flex-col gap-4 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Booking Kunjungan</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Kelola pengajuan kunjungan nasabah ke cabang</p>
        </div>
        <div class="flex items-center gap-3">
            <div class="relative">
                <span class="absolute -translate-y-1/2 left-4 top-1/2 pointer-events-none">
                    <svg class="fill-gray-500 dark:fill-gray-400" width="20" height="20" viewBox="0 0 20 20" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M3.04199 9.37381C3.04199 5.87712 5.87735 3.04218 9.37533 3.04218C12.8733 3.04218 15.7087 5.87712 15.7087 9.37381C15.7087 12.8705 12.8733 15.7055 9.37533 15.7055C5.87735 15.7055 3.04199 12.8705 3.04199 9.37381ZM9.37533 1.54218C5.04926 1.54218 1.54199 5.04835 1.54199 9.37381C1.54199 13.6993 5.04926 17.2055 9.37533 17.2055C11.2676 17.2055 13.0032 16.5346 14.3572 15.4178L17.1773 18.2381C17.4702 18.531 17.945 18.5311 18.2379 18.2382C18.5308 17.9453 18.5309 17.4704 18.238 17.1775L15.4182 14.3575C16.5367 13.0035 17.2087 11.2671 17.2087 9.37381C17.2087 5.04835 13.7014 1.54218 9.37533 1.54218Z" fill=""/></svg>
                </span>
                <input type="text" id="searchInput" value="{{ request('search') }}"
                    placeholder="Cari nasabah atau No CIF..."
            ...
            <a href="{{ route(auth()->user()->role . '.booking.kunjungan') }}?export=1&search={{ request('search') }}&cabang_id={{ request('cabang_id') }}&status={{ request('status', 'menunggu') }}"
    ...
    {{-- Filter --}}
    <div class="px-6 mb-3">
        <form method="GET" action="{{ route(auth()->user()->role . '.booking.kunjungan') }}" class="flex flex-wrap items-center gap-3">
            ...
            @if(request()->filled('cabang_id') || request()->filled('search') || request('status') !== null)
            <a href="{{ route(auth()->user()->role . '.booking.kunjungan') }}"
        ...
        <table class="w-full">
            <thead class="border-t border-y border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">No Booking</th>
                    ...
                    <th class="px-6 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Tgl & Jam Kunjungan</th>
                    <th class="px-6 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Keperluan</th>
                    <th class="px-6 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Tgl Pengajuan</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                @php
                    $statusConfig = [
                        ...
                        'dikonfirmasi' => 'bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500',
                        'ditolak'      => 'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500',
                        'selesai'      => 'bg-blue-50 text-blue-600 dark:bg-blue-500/15 dark:text-blue-400',
                    ];
                @endphp
                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                    <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">{{ $bookings->firstItem() + $index }}</td>
                    <td class="px-6 py-3.5">
                        <span class="font-medium text-theme-sm text-gray-800 dark:text-white/90">{{ $booking->no_booking }}</span>
                    </td>
                    <td class="px-6 py-3.5">
                        <p class="text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ $booking->nasabah->nama ?? '-' }}</p>
                        <p class="text-xs text-gray-400">{{ $booking->nasabah->no_cif ?? '-' }}</p>
                    </td>
                    <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">{{ $booking->branch->nama ?? '-' }}</td>
                    <td class="px-6 py-3.5">
                        <p class="text-theme-sm text-gray-800 dark:text-white/90">{{ $booking->tgl_kunjungan->format('d M Y') }}</p>
                        <p class="text-xs text-gray-400">{{ $booking->jam_kunjungan }} WIB</p>
                    </td>
                    <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">{{ $booking->keperluan }}</td>
                    <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">{{ $booking->created_at->format('d M Y') }}</td>
                    <td class="px-6 py-3.5">
                        <span class="rounded-full px-2 py-0.5 text-theme-xs font-medium {{ $statusConfig[$booking->status] ?? '' }}">
                            {{ ucfirst($booking->status) }}
                        </span>
                    </td>
                    <td class="px-6 py-3.5">
                        <a href="{{ route(auth()->user()->role . '.booking.kunjungan.show', $booking->id) }}"
                ...
                <tr>
                    <td colspan="9" class="px-6 py-10 text-center text-sm text-gray-400 dark:text-gray-500">
                        Tidak ada data booking kunjungan
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <x-common.pagination :paginator="$bookings" />
    </div>
</div>
