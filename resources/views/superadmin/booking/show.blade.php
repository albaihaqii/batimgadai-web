...
<div class="mb-6 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="flex items-center gap-2 mt-3 sm:mt-0">
        <span class="text-xs text-gray-400 dark:text-gray-500">
            {{ count($kategoriList) }} kategori tersedia
        </span>
    </div>
</div>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{{ $master?->keterangan ?? 'Belum ada keterangan' }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                @if($master)
    ...
    <div class="p-6 space-y-6">
        ...
        <div class="rounded-xl border border-gray-100 bg-gray-50 p-5 dark:border-gray-800 dark:bg-gray-900">
            <div class="flex items-center gap-2 mb-4">
                <div class="w-1.5 h-4 rounded-full bg-brand-500"></div>
                <h5 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Persentase Gadai dari Harga Pasar</h5>
                <span class="ml-auto text-xs text-gray-400">Sistem akan menghitung nilai pinjaman dari rentang ini</span>
            </div>
            <form method="POST" action="{{ route('superadmin.simulasi.master.update', $kat) }}"
                ...
                <div class="flex items-center gap-3 pb-0.5">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <div class="relative">
                            <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ ($master?->is_active ?? true) ? 'checked' : '' }}>
            ...
            <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex flex-wrap gap-4 text-xs text-gray-500 dark:text-gray-400">
                ...
                <span class="text-brand-500 font-medium">→ Estimasi pinjaman: Rp {{ number_format(5000000 * $master->persen_min / 100, 0, ',', '.') }} — Rp {{ number_format(5000000 * $master->persen_max / 100, 0, ',', '.') }}</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="rounded-xl border border-gray-100 bg-gray-50 p-5 dark:border-gray-800 dark:bg-gray-900">
                <div class="flex items-center gap-2 mb-4">
                    <div class="w-1.5 h-4 rounded-full bg-error-500"></div>
                ...
                <p class="text-xs text-gray-400 dark:text-gray-500 mb-3">Faktor pengurangan nilai (nilai negatif, contoh: -10)</p>
                ...
                <div class="space-y-2 mb-4 max-h-56 overflow-y-auto pr-1">
                    ...
                    <div class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <span class="text-sm font-bold {{ $cacat->faktor < 0 ? 'text-error-600 dark:text-error-400' : 'text-gray-500' }} min-w-12 text-right">
                            {{ $cacat->faktor > 0 ? '+' : '' }}{{ $cacat->faktor }}%
                        </span>
                        <span class="w-1.5 h-1.5 rounded-full {{ $cacat->is_active ? 'bg-success-500' : 'bg-gray-300' }}"></span>
                        <button onclick="openEditModal('kecacatan', {{ $cacat->id }}, '{{ addslashes($cacat->label) }}', {{ $cacat->faktor }}, {{ $cacat->is_active ? 1 : 0 }})"
                        ...
                        <form method="POST" action="{{ route('superadmin.simulasi.kecacatan.destroy', $cacat->id) }}" class="inline"
                            onsubmit="return confirm('Hapus item ini?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="text-gray-400 hover:text-red-600 transition-colors flex-shrink-0">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                            </button>
                        </form>
                    </div>
                    @empty
                    <div class="rounded-lg border border-dashed border-gray-200 dark:border-gray-700 py-6 text-center">
                        <p class="text-sm text-gray-400">Belum ada item kecacatan</p>
                    </div>
                    @endforelse
                </div>

                {{-- Tambah --}}
                <form method="POST" action="{{ route('superadmin.simulasi.kecacatan.store') }}" class="flex gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                    @csrf
            ...
            <div class="rounded-xl border border-gray-100 bg-gray-50 p-5 dark:border-gray-800 dark:bg-gray-900">
                <div class="flex items-center gap-2 mb-4">
                    <div class="w-1.5 h-4 rounded-full bg-success-500"></div>
                ...
                <p class="text-xs text-gray-400 dark:text-gray-500 mb-3">Faktor penambahan/pengurangan nilai (contoh: +5 atau -5)</p>
                ...
                <div class="space-y-2 mb-4 max-h-56 overflow-y-auto pr-1">
                    ...
                    <div class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <span class="text-sm font-bold {{ $lk->faktor > 0 ? 'text-success-600 dark:text-success-400' : ($lk->faktor < 0 ? 'text-error-600 dark:text-error-400' : 'text-gray-500') }} min-w-12 text-right">
                            {{ $lk->faktor > 0 ? '+' : '' }}{{ $lk->faktor }}%
                        </span>
                        <span class="w-1.5 h-1.5 rounded-full {{ $lk->is_active ? 'bg-success-500' : 'bg-gray-300' }}"></span>
                        <button onclick="openEditModal('kelengkapan', {{ $lk->id }}, '{{ addslashes($lk->label) }}', {{ $lk->faktor }}, {{ $lk->is_active ? 1 : 0 }})"
                            class="text-gray-400 hover:text-brand-500 transition-colors flex-shrink-0">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                        </button>
                        <form method="POST" action="{{ route('superadmin.simulasi.kelengkapan.destroy', $lk->id) }}" class="inline"
                            onsubmit="return confirm('Hapus item ini?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="text-gray-400 hover:text-red-600 transition-colors flex-shrink-0">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                    ...
                    @empty
                    <div class="rounded-lg border border-dashed border-gray-200 dark:border-gray-700 py-6 text-center">
                        <p class="text-sm text-gray-400">Belum ada item kelengkapan</p>
                    </div>
                    @endforelse
                </div>

                {{-- Tambah --}}
                <form method="POST" action="{{ route('superadmin.simulasi.kelengkapan.store') }}" class="flex gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                    @csrf
                    <input type="hidden" name="kategori" value="{{ $kat }}">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                        Faktor (%)
                        <span class="text-xs text-gray-400 font-normal ml-1" id="faktorHint">nilai negatif untuk pengurangan</span>
                    </label>
                    <div class="relative">
...
function openEditModal(tipe, id, label, faktor, isAktif) {
    ...
    document.getElementById('faktorHint').textContent     = isCacat ? 'nilai negatif untuk pengurangan' : 'positif = menambah nilai, negatif = mengurangi';
