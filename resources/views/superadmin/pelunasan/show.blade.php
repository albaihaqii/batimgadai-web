...
<x-common.page-breadcrumb pageTitle="Detail Pelunasan" />
...
<div id="successModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
    <div class="w-full max-w-md rounded-2xl bg-white dark:bg-gray-900 shadow-2xl overflow-hidden">
        <div class="p-6 text-center">
            ...
            <h3 class="text-lg font-bold text-gray-800 dark:text-white mb-2">Pelunasan Berhasil!</h3>
...
@elseif($pelunasan->status_bayar === 'menunggu' && $pelunasan->metode_bayar === 'midtrans' && $pelunasan->midtrans_token)
<div class="mb-6 p-5 rounded-2xl bg-blue-50 border border-blue-200 dark:bg-blue-500/10 dark:border-blue-500/20">
    <div class="flex items-center gap-3 mb-3">
        <svg class="text-blue-600 flex-shrink-0" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z"/>
    ...
    <div class="flex flex-wrap gap-3">
        ...
        <form method="POST"
            action="{{ route(auth()->user()->role . '.transaksi.pelunasan.retry', $pelunasan->id) }}">
            @csrf
            <button type="submit"
...
<script>
    document.getElementById('pay-button').onclick = function() {
        snap.pay('{{ $pelunasan->midtrans_token }}', {
            onSuccess: function(result) { window.location.reload(); },
            onPending: function(result) { window.location.reload(); },
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
    <div class="lg:col-span-2 space-y-6">
            ...
            <div class="px-6 py-5 flex items-center justify-between">
                <div>
                    <h3 class="text-base font-semibold text-gray-800 dark:text-white">
                        {{ $pelunasan->no_sbg ?? 'Pelunasan #' . $pelunasan->id }}
                    </h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                        Diproses oleh {{ $pelunasan->officer->nama ?? '-' }} pada {{ $pelunasan->created_at->format('d M Y, H:i') }} WIB
                    </p>
                </div>
                @php
                    $statusConfig = [
                ...
                @endphp
                <span class="rounded-full px-3 py-1 text-sm font-semibold {{ $statusConfig[$pelunasan->status_bayar] ?? 'bg-gray-100 text-gray-600' }}">
                    {{ ucfirst($pelunasan->status_bayar) }}
                </span>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-2 gap-5">
                    <div>
                        ...
                        <p class="text-sm font-bold text-brand-500 mt-1">{{ $pelunasan->gadai->no_sbg ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Nasabah</p>
                        <p class="text-sm font-semibold text-gray-800 dark:text-white mt-1">{{ $pelunasan->nasabah->nama ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Barang</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white mt-1">{{ $pelunasan->gadai->barang->nama_barang ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Cabang</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white mt-1">{{ $pelunasan->gadai->branch->nama ?? '-' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-800">
                <h3 class="text-base font-semibold text-gray-800 dark:text-white">Detail Pelunasan</h3>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-2 gap-5">
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Pokok Pinjaman</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white mt-1">Rp {{ number_format($pelunasan->nilai_pinjaman, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Biaya Jasa ({{ $pelunasan->jasa_persen }}%)</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white mt-1">Rp {{ number_format($pelunasan->jasa_nominal, 0, ',', '.') }}</p>
                    </div>
                    @if($pelunasan->denda_nominal > 0)
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Denda ({{ $pelunasan->denda_persen }}%)</p>
                        <p class="text-sm font-medium text-error-600 mt-1">Rp {{ number_format($pelunasan->denda_nominal, 0, ',', '.') }} <span class="text-xs">({{ $pelunasan->hari_terlambat }} hari)</span></p>
                    </div>
                    @endif
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Total Tebus</p>
                        <p class="text-sm font-bold text-brand-500 mt-1">Rp {{ number_format($pelunasan->total_tebus, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Tgl Pelunasan</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white mt-1">{{ $pelunasan->tgl_pelunasan->format('d M Y') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Jatuh Tempo</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white mt-1">{{ $pelunasan->tgl_jt->format('d M Y') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Metode Bayar</p>
                        <span class="inline-block mt-1 rounded-full px-2 py-0.5 text-theme-xs font-medium {{ $pelunasan->metode_bayar === 'tunai' ? 'bg-gray-100 text-gray-600' : 'bg-blue-50 text-blue-600' }}">
                            {{ ucfirst($pelunasan->metode_bayar) }}
                        </span>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Status</p>
                        <span class="inline-block mt-1 rounded-full px-2 py-0.5 text-theme-xs font-medium {{ $statusConfig[$pelunasan->status_bayar] ?? 'bg-gray-100 text-gray-600' }}">
                            {{ ucfirst($pelunasan->status_bayar) }}
                        </span>
                    </div>
                </div>
    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-800">
                    ...
                    <span class="text-sm font-medium text-gray-800 dark:text-white">Rp {{ number_format($pelunasan->nilai_pinjaman, 0, ',', '.') }}</span>
                </div>
                <div class="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-800">
                    <span class="text-sm text-gray-500 dark:text-gray-400">Jasa</span>
                    <span class="text-sm font-medium text-gray-800 dark:text-white">Rp {{ number_format($pelunasan->jasa_nominal, 0, ',', '.') }}</span>
                </div>
                @if($pelunasan->denda_nominal > 0)
                <div class="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-800">
                    <span class="text-sm text-error-600">Denda</span>
                    <span class="text-sm font-medium text-error-600">Rp {{ number_format($pelunasan->denda_nominal, 0, ',', '.') }}</span>
                </div>
                @endif
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-bold text-gray-800 dark:text-white">Total Tebus</span>
                    <span class="text-sm font-bold text-brand-500">Rp {{ number_format($pelunasan->total_tebus, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="space-y-3">
                @if($pelunasan->status_bayar === 'berhasil')
                <a href="{{ route(auth()->user()->role . '.transaksi.gadai.sbg', $pelunasan->gadai_id) }}?tipe=pelunasan"
                    target="_blank"
                ...
                @endif
                <a href="{{ route(auth()->user()->role . '.transaksi.gadai.show', $pelunasan->gadai_id) }}"
