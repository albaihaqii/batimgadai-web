...
<x-common.page-breadcrumb pageTitle="Edit Rate Jasa" />

<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-800">
        <h3 class="text-base font-semibold text-gray-800 dark:text-white">Edit Rate Perhitungan Jasa</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Tipe: <span class="font-semibold capitalize text-brand-500">{{ $jasaRate->tipe === 'umum' ? 'Umum' : 'Perhiasan / Emas' }}</span>
        </p>
    </div>

    <form method="POST" action="{{ route('superadmin.jasa-rate.update', $jasaRate->id) }}" class="p-6">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative w-48">
                    <select name="is_active"
                        ...
                        <option value="1" {{ $jasaRate->is_active ? 'selected' : '' }}>Aktif</option>
                        <option value="0" {{ !$jasaRate->is_active ? 'selected' : '' }}>Nonaktif</option>
                    </select>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="min_pinjaman" value="{{ old('min_pinjaman', $jasaRate->min_pinjaman) }}" min="0"
            ...
            <div>
                ...
                <div class="relative">
                    <span class="absolute top-1/2 left-4 -translate-y-1/2 text-sm text-gray-500 pointer-events-none font-medium">Rp</span>
                    <input type="number" name="max_pinjaman" value="{{ old('max_pinjaman', $jasaRate->max_pinjaman) }}" min="0"
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="jasa_15_hari" value="{{ old('jasa_15_hari', $jasaRate->jasa_15_hari) }}"
                        min="0" max="100" step="0.01"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 pr-10 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 {{ $errors->has('jasa_15_hari') ? 'border-red-500' : '' }}">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="jasa_30_hari" value="{{ old('jasa_30_hari', $jasaRate->jasa_30_hari) }}"
                        min="0" max="100" step="0.01"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 pr-10 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 {{ $errors->has('jasa_30_hari') ? 'border-red-500' : '' }}">
                    <span class="absolute top-1/2 right-4 -translate-y-1/2 text-sm text-gray-400 pointer-events-none">%</span>
                </div>
