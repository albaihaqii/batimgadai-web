...
@section('content')
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Hero Beranda</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Kelola daftar slide hero. Maksimal 4 slide boleh aktif bersamaan.
            </p>
        </div>
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('superadmin.settings.hero.create') }}"
            ...
            <table class="w-full min-w-[900px]">
                <thead class="border-y border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">Tombol</th>
                    ...
                    @forelse ($slides as $slide)
                        <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                            ...
                            <td class="px-5 py-3.5">
                                <img src="{{ asset($slide->image_path) }}" alt="{{ $slide->title }}" class="h-14 w-24 rounded-lg object-cover">
                            </td>
                            <td class="px-5 py-3.5">
                                <p class="text-theme-sm font-semibold text-gray-800 dark:text-white/90">{{ $slide->title }}</p>
                                @if ($slide->highlighted_title)
                                    <p class="text-xs text-brand-500">{{ $slide->highlighted_title }}</p>
                                @endif
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('superadmin.settings.hero.edit', $slide) }}" class="text-gray-400 transition-colors hover:text-gray-900 dark:hover:text-white">
                                    ...
                                    <form method="POST" action="{{ route('superadmin.settings.hero.destroy', $slide) }}" onsubmit="return confirm('Hapus slide hero ini?')">
                                        @csrf
                                        @method('DELETE')
