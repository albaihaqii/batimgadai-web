...
<x-common.page-breadcrumb pageTitle="Edit Pimpinan" />

<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-800">
        <h3 class="text-base font-semibold text-gray-800 dark:text-white">Form Edit Pimpinan</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Perbarui data pimpinan dengan lengkap dan benar.</p>
    </div>

    <form method="POST" action="{{ route('superadmin.pimpinan.update', $admin->id) }}" enctype="multipart/form-data" class="p-6">
        @csrf
        @method('PUT')
            ...
            <div class="md:col-span-2">
                ...
                <div class="flex items-center gap-4 mb-3">
                    <div class="w-16 h-16 rounded-full bg-brand-500 flex items-center justify-center overflow-hidden flex-shrink-0">
                        @if($admin->foto)
                            <img src="{{ asset('storage/' . $admin->foto) }}?v={{ time() }}" alt="{{ $admin->nama }}" class="w-full h-full object-cover" id="foto-preview">
                        @else
                            <span class="text-white font-bold text-xl" id="foto-initial">{{ strtoupper(substr($admin->nama, 0, 1)) }}</span>
                            <img src="" alt="" class="w-full h-full object-cover hidden" id="foto-preview">
                        @endif
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $admin->nama }}</p>
                ...
                <div class="relative">
                    <select name="cabang_id"
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}" {{ old('cabang_id', $admin->cabang_id) == $branch->id ? 'selected' : '' }}>
                                {{ $branch->nama }}
                            </option>
                        @endforeach
                    </select>
