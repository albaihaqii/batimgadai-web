...
<div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div class="flex flex-col gap-4 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Data Pimpinan</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Kelola data pimpinan BATIM GADAI</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            {{-- Unduh --}}
            <a href="{{ route('superadmin.pimpinan') }}?export=1&search={{ request('search') }}"
            ...
            {{-- Tambah --}}
            <a href="{{ route('superadmin.pimpinan.create') }}"
    ...
    {{-- Entries --}}
    <div class="px-6 mb-3">
        <form method="GET" action="{{ route('superadmin.pimpinan') }}" class="flex items-center gap-2">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-6 py-3.5">
                        <div class="w-10 h-10 rounded-full overflow-hidden bg-brand-500 flex items-center justify-center flex-shrink-0">
                            @if($admin->foto)
                                <img src="{{ asset('storage/' . $admin->foto) }}" alt="{{ $admin->nama }}" class="w-full h-full object-cover">
                            @else
                                <span class="text-white font-bold text-sm">{{ strtoupper(substr($admin->nama, 0, 1)) }}</span>
                            @endif
                        </div>
                    </td>
                    <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">{{ $admin->nama }}</td>
                    <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">{{ $admin->email }}</td>
                    <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">{{ $admin->branch->nama ?? '-' }}</td>
                    <td class="px-6 py-3.5 text-theme-sm text-gray-500 dark:text-gray-400">{{ $admin->created_at->format('d M Y') }}</td>
                    <td class="px-6 py-3.5">
                    ...
                    <td class="px-6 py-3.5">
                        <div class="flex items-center gap-3">
                            {{-- Edit --}}
                            <a href="{{ route('superadmin.pimpinan.edit', $admin->id) }}"
                            ...
                            {{-- Hapus --}}
                            <button onclick="openDeleteModal({{ $admin->id }}, '{{ $admin->nama }}')"
    ...
    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-sm text-gray-500 dark:text-gray-400">
            Menampilkan {{ $admins->firstItem() ?? 0 }}–{{ $admins->lastItem() ?? 0 }} dari {{ $admins->total() }} data
        </p>
        <div class="flex items-center gap-2">
            <a href="{{ $admins->previousPageUrl() }}"
                class="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 {{ $admins->onFirstPage() ? 'opacity-40 pointer-events-none' : '' }}">
            ...
            <a href="{{ $admins->nextPageUrl() }}"
                class="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 {{ !$admins->hasMorePages() ? 'opacity-40 pointer-events-none' : '' }}">
...
<div id="modalHapus" class="hidden fixed inset-0 z-99999 flex items-center justify-center bg-black/50">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md mx-4">
        <div class="p-6 text-center">
            ...
            <h4 class="text-lg font-semibold text-gray-800 dark:text-white mb-2">Hapus Data Pimpinan</h4>
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
                Apakah Anda yakin ingin menghapus data pimpinan <span id="deleteNama" class="font-semibold text-gray-800 dark:text-white"></span>? Tindakan ini tidak dapat dibatalkan.
            </p>
...
function openDeleteModal(id, nama) {
    ...
    document.getElementById('formHapus').action = `/superadmin/pimpinan/${id}`;
